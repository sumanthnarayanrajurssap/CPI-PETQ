...
for /f %%G IN ('dir %USERPROFILE%\supaData\* /b /ad-h /od') do set "latestDir=%%~G"

ECHO latest folder = %latestDir%
if not exist "%1\latestResult" mkdir %1\latestResult
ECHO going to copy
xcopy %USERPROFILE%\supaData\%latestDir% %1\latestResult /O /X /E /H /K /R
ECHO Should have copied
rmdir /s /q %USERPROFILE%\supaData\%latestDir%
ECHO should have deleted
ECHO final folder directory = %1\latestResult
